...
import {formatCurrency, formatLongDate, formatSmallDate} from "../../../util/format";
...
const headers = [
    ...
    },
    {
        ...
        sortable: true,
        align: 'end',
        class: 'min-w-[160px] max-w-[160px] text-end',
    },
    {
        name: 'createdAt',
        label: 'Ngày đăng',
    ...
    return (
        <div className="w-full border-b border-b-secondary-bg border-dashed">
            <div className="flex items-center gap-6 p-3">
                <div className="w-[28px] max-w-[28px] text-center font-semibold text-md"></div>
                {headers.map(header => (
...
export default function ProductTable({products, pagination, setPagination}) {
    ...
    const handleExpand = (id) => {
        ...
        if (isOpen)
            protectedRequest().get(`/products/${id}`).then(res => {
    ...
    return (
        <div>
            <div className="flex items-center gap-5 flex-wrap justify-between mb-5">
                ...
                <div className="flex items-center justify-end gap-5">
                    <Link to={`/kenh-ban-hang/san-pham/dang-ban`}
                          className="flex items-center gap-1.5 rounded-full bg-primary-bg text-primary text-md font-semibold px-3 py-2">
                        <UilPlusCircle className="w-[20px] h-[20px]"/>
                        <span>Đăng sản phẩm mới</span>
                    </Link>
                    <form
                        className={"w-[250px] flex items-center gap-3 rounded-md bg-secondary-bg text-secondary text-md font-medium px-3 py-2"}>
            ...
            <div className="mb-3">
                {products.map((product, i) => {
                    ...
                    let index = 0;
                    return (
                        <div key={i} className="border-b border-b-secondary-bg border-dashed">
                            <div
                                className={`${expandProduct.length > 0 ? 'bg-app-1' : 'bg-white'} hover:bg-app-1 rounded-md`}>
                                <div className="flex items-center gap-6 p-3 text-tiny">
                                    ...
                                    <div className={`${headers[index++].class} font-semibold text-base text-primary`}>
                                    ...
                                    <div className={`${headers[index++].class} font-medium text-tiny`}>
                                        {formatSmallDate(product.createdAt)}
                                    </div>
                                    <div
                                                            ...
                                                            <div key={a.id} className="mb-5">
                                                                <div className="flex items-start gap-3">
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        {a.options.map(o => (
                                                                            <div key={o.id}
                                                                                 className="text-md font-semibold text-secondary bg-secondary-bg rounded-md px-3 py-1">
                                                                                {o.name}
                                                                            </div>
                                                                        ))}
                                                                    </div>
                                                ...
                                                {p.returnPolicies?.length > 0 &&
                                                    <div className="">
                                                        ...
                                                        {p.returnPolicies.map(policy => (
                                                            <Tooltip title={policy.tooltipContent} followCursor arrow
                                                                     key={policy.id}>
                                                                <div
